            ...
            <div className="rounded-2xl border border-gold/15 bg-white/80 dark:bg-white/[0.04] backdrop-blur-xl shadow-xl-soft p-6 md:p-8">
              <form onSubmit={submit} className="flex flex-col gap-4">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="mt-2 w-full px-6 py-3 rounded-xl bg-gradient-to-r from-saffron to-gold text-white text-sm font-semibold tracking-wide shadow-glow-saffron hover:shadow-lg hover:shadow-saffron/30 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {loading ? 'Authenticating...' : (mode === 'login' ? 'Sign In' : 'Create Account')}
                  {!loading && <ArrowRight size={15} className="transition-transform duration-300" />}
                </motion.button>
              </form>
              ...
              <div className="grid grid-cols-1 gap-3">
                <motion.button
                  ...
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-gold/20 bg-white/60 dark:bg-white/[0.03] text-xs font-medium text-ink/70 dark:text-sand-lt/70 hover:bg-saffron/5 hover:border-saffron/30 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
